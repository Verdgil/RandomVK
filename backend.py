...
class vk_audio:
    ...
    def get_audio(self):
        ...
        while not do:
            try:
                audio_user = self.api.audio.get(user_id=user_id)
                ...
                if len(audio_user) > 0:
                    ...
                    user_id = randint(1, 5000000)
        lastfm = last()
        audio_num = randint(0, len(audio_user) - 1)
        self.now_play_audio_id['id'] = user_id
        self.now_play_audio_id['aid'] = audio_user[audio_num]['aid']
        artist = audio_user[audio_num]['artist']
        title = audio_user[audio_num]['title']
        image = lastfm.get_image(artist, title)
        if image == lastfm.Not_Found:
            audio = {'url': audio_user[audio_num]['url'],
                     'artist': artist,
                     'title': title,
                     'id': user_id,
                     'aid': audio_user[audio_num]['aid'],
                     'duration': audio_user[audio_num]['duration']}
            return audio
        audio = {'url': audio_user[audio_num]['url'],
                 'artist': artist,
                 'title': title,
